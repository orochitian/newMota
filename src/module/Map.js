import map00 from './maps/map00';
import map01 from './maps/map01';
import map02 from './maps/map02';
import map03 from './maps/map03';
import map04 from './maps/map04';
import map05 from './maps/map05';
import map06 from './maps/map06';
import map07 from './maps/map07';
import map08 from './maps/map08';
import map09 from './maps/map09';
import map10 from './maps/map10';
import map11 from './maps/map11';
import map12 from './maps/map12';
import map13 from './maps/map13';
import map14 from './maps/map14';
import map16 from './maps/map16';
import map15 from './maps/map15';
import map17 from './maps/map17';
import map18 from './maps/map18';

export default [
    map00, map01, map02, map03, map04, map05, map06,
    map07, map08, map09, map10, map11, map12, map13,
    map14, map15, map16, map17, map18
]