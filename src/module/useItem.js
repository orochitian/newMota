...
export default function (core) {
    ...
    var flows = core.flows;
    //  怪物手册
    getId('monsterMenu').onclick = function () {
        var monsterNames = [];
        var menuHtml = ``;
        if( hero.monsterMenu < 1 ) {
            tools.msgRender('未获得怪物手册');
            return;
        }
        if( map.monsters.length < 1 ) {
            tools.eventRender('没有怪兽了');
            return;
        }
        for( var i=0; i<map.monsters.length; i++ ) {
            var monster = addMonster(map.monsters[i][0], map.monsters[i][1]);
            if( monsterNames.indexOf(monster.name) === -1 ) {
                ...
                menuHtml += `
                    <p>
                        ...
                        血量：<span class="brown margin-lr">${monster.hp}</span> 
                        攻击：<span class="brown margin-lr">${monster.attack}</span> 
                        防御：<span class="brown margin-lr">${monster.defense}</span>
                    </p>
                    <p style="margin-left: 50px;">
                        伤害：<span class="green margin-lr">${damage}</span>
                        金币：<span class="brown margin-lr">${monster.money}</span>
                    </p>
                `
            }
        }
        tools.eventRender(menuHtml)
    };
